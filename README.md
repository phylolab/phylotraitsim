...
The script above will use the parameters from the file `seed_rates_3.txt` and parse them to `logistic.R` and `simulator0.3.R`. These parameters (`p_a` and `p_b`) contain a seed and three different $\sigma_E$'s corresponding to three evolutionary rate scenarios. A total of 100 simulations are run per scenario, thus, the file `seed_rates_3.txt` has 300 lines.
